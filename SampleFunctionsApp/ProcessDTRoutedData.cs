...
namespace SampleFunctionsApp
{
    ...
    public static class ProcessDTRoutedData
    {
        ...
        {
            log.LogInformation($"ProcessDTRoutedData Fired!");
                    ...
                    if (parentId != null)
                    {
                        ...
                        foreach (var operation in message["data"]["patch"])
                        {
                            ...
                            if (opValue.Equals("replace"))
                            {
                                ...
                                if (propertyPath.Equals("/Moisture"))
                                {
                                    log.LogInformation($"PropertyPath: {propertyPath}");
                                    await AdtUtilities.UpdateTwinPropertyAsync<double>(client, parentId, propertyPath, operation["value"].Value<double>(), log);
                                }
                            }
                        }
                    }
